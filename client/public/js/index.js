...
// login
{
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    var pwdInput = document.querySelector(
      ".signinForm .userForm input[name=password]"
    );
    var userInput = document.querySelector(
      ".signinForm .userForm input[name=username]"
    );
...
// Đăng ký
{
  ...
  signupForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    var userInput = document.querySelector(
      ".signupForm .userForm input[name=username]"
    );
    var pwdInput = document.querySelector(
      ".signupForm .userForm input[name=password]"
    );
    var confirmPwdInput = document.querySelector(
      ".signupForm .userForm input[name=confirmPassword]"
    );
    var nameInput = document.querySelector(
      ".signupForm .userForm input[name=name]"
    );
    if (confirmPwdInput.value !== pwdInput.value) {
      document.querySelector(".signupForm .userForm .warning-text").innerHTML =
    ...
    if (jwtoken.success) {
      ...
      document
        .querySelectorAll(".signupForm .userForm input")
        .forEach((input) => {
          input.value = "";
        });
      toSignInForm();
      ...
      document.querySelector(
        ".signupForm .userForm .warning-text"
      ).innerHTML = `${jwtoken.message}`;
    }
  });
}
...
window.reloadCakes = async function reloadCakes(
  sweetCakes,
  saltCakes,
  breads,
  isAdmin
...
window.onload = async () => {
  ...
  if (!localStorage.getItem("cakes")) {
    ...
    localStorage.setItem(
      "cakes",
      JSON.stringify({ sweetCakes, saltCakes, breads })
    );
    await reloadCakes(sweetCakes, saltCakes, breads, isAdmin);
    // loadCartProduct();
  } else {
    const { sweetCakes, saltCakes, breads } = JSON.parse(
  ...
  if (isAdmin) {
    ...
    const addForm = document.querySelector(
      ".cake-add-formContainer.add .cake-add-form"
    );
    addForm.onsubmit = async (event) => {
      event.preventDefault();
      const cakeAddName = document.querySelector(
        ".cake-add-formContainer.add input[name=name]"
      );
      const cakeAddPrice = document.querySelector(
        ".cake-add-formContainer.add input[name=price]"
      );
      const cakeAddImage = document.querySelector(
        ".cake-add-formContainer.add input[name=image]"
      );
      const cakeAddUnit = document.querySelector(
        ".cake-add-formContainer.add select[name=unit]"
      );
      const cakeAddDesc = document.querySelector(
        ".cake-add-formContainer.add textarea"
      );
      const cakeAddGroupId = document.querySelector(
        ".cake-add-formContainer.add input[name=group_id]"
      );
      const cakeAddTypeId = document.querySelector(
        ".cake-add-formContainer.add input[name=type_id]"
      );
      ...
      if (result.success) {
        ...
        document
          .querySelector(".cake-add-formContainer.add")
...
function createCartList(allCakes) {
  ...
  cartAddBtns.forEach((btn) => {
    btn.onclick = (e) => {
      e.preventDefault();
      const { cake_id, name, price, image } = allCakes.find(
        (cake) => cake.cake_id === e.target.parentNode.id
      );
      ...
      if (localCart) {
        ...
        localStorage.setItem(
          "cart",
          JSON.stringify([{ cake_id, name, price, image, quantity: 1 }])
        );
        document.querySelector(".cart-product-container").innerHTML = addToCart(
    ...
    decr.onclick = (e) => {
      ...
      sum.innerHTML =
        threeDigitNumber(
          ...
          Number(prod.querySelector(".price").innerHTML.slice(0, -1)) *
          1000
        ) + " đ";
  ...
  cart.forEach((product) => {
    ...
    <div class="cart-product" id="${product.cake_id}">
      ...
      <div class="cart-product-info">
        ...
        <div onclick="eraseProductInCart('${product.cake_id
      }')" class="cart-product-delete"><i style="pointer-events: none" class="fa-solid fa-trash"></i></div>
      </div>
      ...
      <span class="quantity-value">${product.quantity}</span>
        ...
        <div class="sum">
          <h3>Thành tiền: </h3>
          <div class="sum-value">${threeDigitNumber(
...
function checkUser() {
  ...
  if (token) {
    ...
    if (payload.exp > Date.now() / 1000) {
      ...
      document.querySelectorAll(".user-name").forEach((namespace) => {
        namespace.innerHTML = payload.name;
      });
        ...
        if (data.success) {
          ...
          if (curData) {
            ...
            if (data.cake.group_id.trim() === "G001") {
              curData.sweetCakes = curData.sweetCakes.filter(
                (cake) => cake.cake_id.trim() !== data.cake.cake_id.trim()
              );
            } else if (data.cake.group_id.trim() === "G002") {
              curData.saltCakes = curData.saltCakes.filter(
                (cake) => cake.cake_id.trim() !== data.cake.cake_id.trim()
              );
            } else {
              curData.breads = curData.breads.filter(
                (cake) => cake.cake_id.trim() !== data.cake.cake_id.trim()
              );
            }
...
function cakeAddBtns() {
  ...
  cakeAddBtns.forEach((btn) => {
    btn.onclick = async (e) => {
      e.preventDefault();
      e.target.parentNode
        .querySelector(".cake-add-formContainer")
      ...
      e.target.parentNode.querySelector("form").onsubmit = async (ev) => {
        ...
        const cakeGroupId = ev.target.querySelector(
          "input[name=group_id]"
        ).value;
        const cakeTypeId = ev.target.querySelector(
          "select[name=type_id]"
        ).value;
        ...
        if (data.success) {
          ...
          if (curData) {
            ...
            localStorage.setItem("cakes", JSON.stringify(curData));
            ev.target
              .querySelector(".cake-add-close-btn")
              .dispatchEvent(new Event("click"));
          }
  ...
  const closeCakeAddBtns = document.querySelectorAll(
    ".cake-add-container .cake-add-close-btn"
  );

  closeCakeAddBtns.forEach(
    (btn) =>
    (btn.onclick = (e) => {
...
function cakeEditBtns() {
  ...
  editBtns.forEach((btn) => {
    btn.onclick = async (e) => {
      ...
      document
        .querySelector(`.cake-card[id='${curId}'] .cake-add-formContainer`)
        .classList.add("active");
      const response = await fetch(`/cake/${curId}`);
      const cake = await response.json();

      const cakeEditName = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-formContainer input[name=name]`
      );
      const cakeEditPrice = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-formContainer input[name=price]`
      );
      const cakeEditImage = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-formContainer input[name=image]`
      );
      const cakeEditUnit = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-formContainer select[name=unit]`
      );
      const cakeEditDesc = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-formContainer textarea`
      );
      const cakeEditGroupId = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-formContainer input[name=group_id]`
      );
      const cakeEditTypeId = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-formContainer select[name=type_id]`
      );

      const cakeEditForm = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-formContainer.edit .cake-add-form`
      );
      ...
      cakeEditForm.onsubmit = async (event) => {
        ...
        if (result.success) {
          ...
          const { isAdmin } = checkUser();
          await reloadCakes(
            cakesInCache.sweetCakes,
            cakesInCache.saltCakes,
            cakesInCache.breads,
            isAdmin
          );
      ...
      const closeCakeEditBtn = document.querySelector(
        `.cake-card[id='${curId}'] .cake-add-close-btn`
      );
...
async function cakeForUser(cakes, isAdmin, cakeType) {
  ...
  let currentGroupTypes = allTypes.filter(
    (type) => type.group_id === cakeWrapper.id
  );
  console.log("IsAdmin: ", isAdmin)
  let cakeAddForm = isAdmin
    ? `        <div class="cake-add-formContainer add">
      ...
      (type) =>
        `<option value="${type.type_id}">${type.type_name}</option>`
            ...
            <div class="cake-add-form-input">
              <input type="text" id="cake-type" disabled  placeholder="${cakeWrapper.getAttribute(
      "data-type"
    )}">
  ...
  cakes.forEach((cake) => {
    cakeWrapper.innerHTML += `
        ...
        <div class="cake-card" id=${cake.cake_id}>
          ${isAdmin
        ? `<div class="cake-add-formContainer edit">
...
`
        : ``
          ...
          <div class="price">${threeDigitNumber(cake.price)} VND / ${cake.unit
      }</div>
          ${isAdmin
        ? `<div class="cart-edit">
